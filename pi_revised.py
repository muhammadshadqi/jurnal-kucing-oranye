...
def build_s5(wb, d):
    ...
    for seg in segs_5:
        ...
        for pi, pr in enumerate(DIRS):
            ...
            row_tot = len(row_sub)
            # col 1 (BL name): only write on first row, merge spans 3 rows
            if pi == 0:
                c1 = ws.cell(row=r, column=1, value=seg)
                c1.font = Font(name="Calibri", bold=True, size=9)
                c1.fill = PatternFill("solid", fgColor=seg_lt)
                c1.alignment = Alignment(horizontal="center", vertical="center")
                ws.merge_cells(start_row=r, start_column=1, end_row=r+2, end_column=1)
            # col 2 (Astro Price direction label)
            sc(ws, r, 2, pr, bg="F8F9FA", align="center", fs=9)
            # cols 3-5 (Down/Stay/Up %)
            for ci, cr in enumerate(DIRS):
