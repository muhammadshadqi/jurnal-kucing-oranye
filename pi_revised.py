...
MG_LBL  = ["A. < -20%","B. -20% to -10%","C. -10% to 0%","D. 0% to 10%",
...
def decompose(d):
    ...
    return {
        "A":A,"B":B,"C":C,"D":D,"E":E,
        "eff_dep":eff_dep,"eff_price":ep,"eff_comp":ec,"eff_new":en,
...
def exact_contributions(seg_res, overall):
    ...
    # Verify sums
    for eff in ["eff_dep","eff_price","eff_comp","eff_new"]:
...
def build_s1(wb, d, period_col, next_col):
    ...
    col_labels = {
        ...
        "eff_price":"Eff Price","eff_comp":"Eff Comp",
    ...
    groups = {
        ...
        "Margin":     ["margin_prev","margin_cur","diff_margin","diff_margin_pct",
                       "margin_pct_prev","margin_pct_cur"],
        "Comp Price": ["comp_price","next_comp_price","diff_comp","diff_comp_pct","comp_status"],
        "PI":         ["pi","next_pi","diff_pi","diff_pi_pct"],
        "Effects":    ["eff_price","eff_comp"],
...
def build_s1b(wb, ov, sr, contribs):
    ...
    for lbl, key in [("1. Effect Churned SKU","eff_dep"),("2.1 Effect Price Change","eff_price"),
                     ("2.2 Effect Comp Change","eff_comp"),
                     ("Effect New SKU","eff_new"),("Total Delta","total")]:
...
def build_s2(wb, addr, ov, sr, contribs):
    ...
    def _data_row(r, label, key, is_base=False, is_res=False, is_delta=False, is_contrib=False):
        ...
        # Hardcoded values
        if key == "eff_existing":
            data_vals = [ov.get("eff_price",0) + ov.get("eff_comp",0)] + \
                        [sr[s].get("eff_price",0) + sr[s].get("eff_comp",0) for s in segs]
            bg = "FFFFF0"; bld = True
            sc(ws, r, 1, label, bold=True, bg="FFFFF0", fc="000000", ind=1, bdr=bb())
        elif is_contrib:
    ...
    for label, key, ib, ir, id_ in [
        ...
        ("1. Effect Churned SKU",       "eff_dep",     False, False, False),
        ("2. Effect Existing SKU",      "eff_existing",False, False, False),
        ("  2.1 Effect Price Change",   "eff_price",   False, False, False),
        ("  2.2 Effect Comp Change",    "eff_comp",    False, False, False),
        ("3. Effect New SKU",           "eff_new",     False, False, False),
    ...
    for label, key, wkey in [
        ("1. Effect Churned SKU",  "eff_dep",   "w_cur"),
        ("2.1 Effect Price Change",   "eff_price", "w_ex"),
        ("2.2 Effect Comp Change",    "eff_comp",  "w_ex"),
        ("Effect New SKU",        "eff_new",   "w_nxt"),
    ]:
...
def _dim_sheet(ws, d, dim_col, dim_label):
    ...
    hdrs = [dim_label,"n Existing","n Departing","n New",
            ...
            "Eff Dep","Eff Price","Eff Comp","Eff New"]
    hdr(ws, 1, hdrs, h=30)
    ws.freeze_panes = "B2"
...
def _matrix_3x3(ws, d, title_text, start_row=1, seg=None):
    ...
    for ri, pr in enumerate(DIRS):
        if ri==0:
            ...
            sc(ws,r,1,"Astro Price",bold=True,bg=seg_lt,align="center",wrap=True)
...
def build_s5(wb, d):
    ...
    # ── SUMMARY TABLE (top) — per BL: Astro Price direction × Comp direction ──
    DIRS = ["Down","Stay","Up"]
    ex_all = d[d["sku_type"]=="Existing"].copy()
    segs_5 = ["Dry","Fresh","Frozen"]

    # Title
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=8)
    c = ws.cell(row=1, column=1, value="Summary — Astro Price vs Comp Movement per Business Line")
    ...
    # Header row
    r = 2
    for i, h in enumerate(["Business Lines","","Down","Stay","Up","Total %"], 0):
        sc(ws, r, 1+i, h, bold=True, bg=NAVY_LT, fc=WHITE, align="center", fs=9)
    ws.row_dimensions[r].height = 20
    r += 1

    for seg in segs_5:
        ex_seg = d[(d["sku_type"]=="Existing") & (d["pricing_bl_25"]==seg)].copy()
        total_seg = len(ex_seg)
        _, seg_lt = SEG_COLORS.get(seg, SEG_COLORS["Overall"])

        for pi, pr in enumerate(DIRS):
            row_sub = ex_seg[ex_seg["price_tag"]==pr]
            row_tot = len(row_sub)
            if pi == 0:
                ws.merge_cells(start_row=r, start_column=1, end_row=r+2, end_column=1)
                sc(ws, r, 1, seg, bold=True, bg=seg_lt, align="center", fs=9)
            sc(ws, r, 2, "Astro Price" if pi==0 else "", bold=(pi==0), bg=seg_lt, align="center", fs=9)
            sc(ws, r, 3-1+1, pr, bg="F8F9FA", align="center", fs=9)  # label col shifted
            # Actually: col1=BL, col2=AstroPrice label, col3=Down, col4=Stay, col5=Up, col6=Total%
            sc(ws, r, 2, pr, bg="F8F9FA", align="center", fs=9)
            sc(ws, r, 1, seg if pi==0 else "", bold=(pi==0), bg=seg_lt, align="center", fs=9)
            for ci, cr in enumerate(DIRS):
                cnt = len(row_sub[row_sub["comp_tag"]==cr])
                pct = cnt/total_seg if total_seg>0 else 0
                bg = seg_lt if (pr==cr) else "FFFFFF"
                sc(ws, r, 3+ci, pct, bg=bg, align="right", fmt="0.0%", fs=9)
            sc(ws, r, 6, row_tot/total_seg if total_seg>0 else 0,
               bold=True, bg="F0F3FA", align="right", fmt="0.0%", fs=9)
            r += 1

        # Total row per segment
        sc(ws, r, 1, "Total", bold=True, bg=NAVY, fc=WHITE, fs=9)
        sc(ws, r, 2, "", bg=NAVY)
        for ci, cr in enumerate(DIRS):
            cnt = len(ex_seg[ex_seg["comp_tag"]==cr])
            sc(ws, r, 3+ci, cnt/total_seg if total_seg>0 else 0,
               bold=True, bg=NAVY, fc=WHITE, align="right", fmt="0.0%", fs=9)
        sc(ws, r, 6, 1.0, bold=True, bg=NAVY, fc=WHITE, align="right", fmt="0.0%", fs=9)
        r += 2

    r += 1  # gap before full matrices

    # ── FULL 3x3 MATRICES ────────────────────────────────────────────────────────
    r = _matrix_3x3(ws, d, "Price vs Comp Movement Matrix", r, seg="Overall")
    for seg in segs_5:
...
def build_s12(wb, ov, sr):
    ...
    for lbl,rng in [("A. < -20%","< -20%"),("B. -20% to -10%","-20% to -10%"),
                    ("C. -10% to 0%","-10% to 0%"),("D. 0% to 10%","0% to 10%"),
                    ("E. 10% to 20%","10% to 20%"),("F. 20% to 30%","20% to 30%"),
                    ("G. 30% to 50%","30% to 50%"),("H. > 50%",">= 50%")]:
        _f(r,lbl,rng); r+=1
    r+=1
    ...
    _f(r,"2.1 Effect Price Change",   "((PI_mid_B - PI_prev) + (PI_cur - PI_mid_D)) / 2",
       "Shapley: rata-rata dua urutan midpoint — zero residual, tidak ada order bias"); r+=1
    _f(r,"2.2 Effect Comp Change",    "((PI_cur - PI_mid_B) + (PI_mid_D - PI_prev)) / 2",
       "Shapley: rata-rata dua urutan midpoint — Ep + Ec = ΔPI selalu exact"); r+=2
    ...
    _f(r,"eff_comp", "Avg(effect_comp)  [existing only] — Shapley"); r+=1
...
def main():
    ...
    print(f"📅  Period type: {period_type.upper()}")

    d = enrich(df_raw)

    p1 = str(d[period_col].dropna().iloc[0])
    p2 = str(d[next_col].dropna().iloc[0])
    out_name = f"PI_Analysis_{period_type}_{p1}_vs_{p2}.xlsx"
    ...
    build_s1b_result = build_s1b(wb, ov, sr, contribs)
    build_s2(wb, build_s1b_result, ov, sr, contribs)
    build_s3(wb, d)
    build_s4(wb, d)
    build_s5(wb, d)
    build_s6(wb, d)
...
def run_pi(file_bytes, filename):
    import io, time
    import pandas as pd
    ...
    df_raw = pd.read_csv(buf) if ext == "csv" else pd.read_excel(buf)
