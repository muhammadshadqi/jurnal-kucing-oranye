...
def write_s0(wb, p1, p2):
    ...
    wsec("4. MARGIN BRIDGE — pp PER STEP", [
        ("1. Churned SKU Effect",
         ...
         "GP & GV existing P1, dep P1","(0.0057pp)","+ = dep margin jelek (cleaning)"),
        ("2.1 COGS Effect",
...
def write_exec(wb, df, pvm, p1, p2):
    ...
    total_pp = sum(pp_val for _,pp_val,_ in bridge_steps)
    ...
    for label_,key_,bg__ in [
        ("1. Churned SKU Effect",  'pp_B',      C_RED),
        ("2.1 COGS Effect", 'pp_cogs',   "FFF3E0"),
        ("2.2 Price Effect",'pp_price',  "E3F2FD"),
        ("Vol/Mix",     'pp_volmix', "EDE7F6"),
        ("3. New SKU Effect",     'pp_G',      "E8F5E9"),
    ]:
        bl_row(label_,[pvm[bl][key_]*100 for bl in BLS5],'0.00"pp"',is_pp=True,bg_=bg__)
...
def write_s1b(wb, pvm, p1, p2):
    ...
    # ── D: PVM Components Rp ─────────────────────────────────────────────────
    hdrs_D = ["BL","GP Start","GP End","1. Churned SKU Effect","2.1 COGS Effect",
              "2.2 Price Effect","2.3 Vol/Mix Effect","3. New SKU Effect","Total Change",
              "Dep Rp%","COGS Rp%","Price Rp%","Vol/Mix Rp%"]
    rows_D = []
    for bi, bl in enumerate(BLS+['TOTAL']):
        d = pvm[bl]
        bg_ = C_TOTAL if bl=='TOTAL' else (C_GREY if bi%2==0 else C_WHITE)
        ...
        gs = d['gp_start']
        rows_D.append((bl,
            [bl, gs, d['gp_end'], -d['gp_dep'], d['cogs_rp'], d['price_rp'],
            ...
            bg_, bld))
    sec(ws, row, 13, "D. PVM COMPONENTS (Rp) — 1.Churned → 2.1COGS → 2.2Price → 2.3Vol/Mix → 3.New")
    row += 1
    ...
    for bl_key, vals, bg_, bld in rows_D:
        ...
        for ci, v in enumerate(vals, 1):
            fmt_ = F_NUM if 2 <= ci <= 9 else (F_PCT if ci >= 10 else None)
            st(ws, row, ci, v, bold=bld, bg=bg_, fmt=fmt_,
               align="right" if ci > 1 else "left")
        row += 1
    row += 1
...
def write_s2(wb, pvm, AGG, p1, p2):
    ...
    title_row(ws, row, NCOLS,
        f"MARGIN BRIDGE — {p1} vs {p2}  |  1.Churned → 2.1COGS → 2.2Price → 2.3Vol/Mix → 3.New")
    row += 1
    ...
    # ── Tabel 1 (Baru) — Summary Margin Bridge per BL (% and IDR%) ───────────────────────────
    # Image 3: columns = per BL (Dry, Fresh, Frozen, PL, Overall) × (%, IDR%)
    # (%)   = pp margin change per effect
    # (IDR%) = GP diff of that effect / GP start of that BL
    BLS5 = BLS + ['TOTAL']
    NCOLS4 = 1 + len(BLS5) * 2
    sec(ws, row, NCOLS4,
        f"Tabel 1 — Summary Margin Bridge per BL  |  (%) = pp margin change  |  (IDR%) = GP effect / GP Start per BL")
    row += 1

    # Row 1 header: BL names spanning 2 cols each
    st(ws, row, 1, "Step", bold=True, bg=C_MID, fc="FFFFFF", align="center")
    for ci, bl in enumerate(BLS5, 0):
        col_start = 2 + ci * 2
        ws.merge_cells(start_row=row, start_column=col_start, end_row=row, end_column=col_start+1)
        lbl = "Overall" if bl == 'TOTAL' else bl
        bg_h = C_DARK if bl == 'TOTAL' else C_MID
        c = ws.cell(row=row, column=col_start, value=lbl)
    ...
    # Row 2: (%) and (IDR%) sub-headers
    hdr(ws, row, 1, "Step")
    for ci in range(len(BLS5)):
        col = 2 + ci * 2
        hdr(ws, row, col,   "(%)")
        hdr(ws, row, col+1, "(IDR%)")
    ws.row_dimensions[row].height = 20
    row += 1

    # Data rows
    t4_rows = [
        (f"P1 Margin — ({p1})",   'base',       C_TOTAL, True),
        ("1. Churned SKU Effect",  'ppB',        C_RED,   False),
        ("2. Existing SKU Effect", 'ppExisting', "FFFFF0",True),
        ...
        ("Total Margin Change",    'ppTot',      C_TOTAL, True),
        (f"P2 Margin — ({p2})",   'end',        C_TOTAL, True),
    ]

    pp_col4 = {'ppB':8,'ppCOGS':9,'ppPrice':10,'ppVM':11,'ppG':12}
    ...
    for label, src, bg_, bld in t4_rows:
        st(ws, row, 1, label, bold=bld, bg=bg_)
        for ci, bl in enumerate(BLS5, 0):
            col_pct = 2 + ci * 2
            col_idr = col_pct + 1
            rCbl = rC[bl]; rAbl = rA[bl]; rEbl = rE[bl]; rDbl = rD[bl]
            gp_start_ref = f"'{A[1:-1]}'!D{rAbl}"

            if src == 'base':
                f_pct = f"='{A[1:-1]}'!J{rAbl}"
                st(ws, row, col_pct, f_pct, fmt=F_PCT, align="right", bold=bld, bg=bg_)
                st(ws, row, col_idr, "—", align="center", fc="888888", bg=bg_)
            elif src == 'end':
                f_pct = f"='{A[1:-1]}'!K{rAbl}"
                st(ws, row, col_pct, f_pct, fmt=F_PCT, align="right", bold=bld, bg=bg_)
                st(ws, row, col_idr, "—", align="center", fc="888888", bg=bg_)
            elif src == 'ppTot':
                f_pct = f"='{A[1:-1]}'!K{rAbl}-'{A[1:-1]}'!J{rAbl}"
                f_idr = f"='{A[1:-1]}'!I{rDbl}/IF({gp_start_ref}<>0,{gp_start_ref},1)"
                st(ws, row, col_pct, f_pct, fmt=F_PCT, align="right", bold=bld, bg=bg_)
                st(ws, row, col_idr, f_idr, fmt=F_PCT, align="right", bold=bld, bg=bg_)
            elif src == 'ppExisting':
                f_pct = (f"='{A[1:-1]}'!{cl(9)}{rCbl}+'{A[1:-1]}'!{cl(10)}{rCbl}"
                         f"+'{A[1:-1]}'!{cl(11)}{rCbl}")
                gp_ex = (f"'{A[1:-1]}'!E{rDbl}+'{A[1:-1]}'!F{rDbl}+'{A[1:-1]}'!G{rDbl}")
                f_idr = f"=({gp_ex})/IF({gp_start_ref}<>0,{gp_start_ref},1)"
                st(ws, row, col_pct, f_pct, fmt=F_PCT, align="right", bold=bld, bg=bg_)
                st(ws, row, col_idr, f_idr, fmt=F_PCT, align="right", bold=bld, bg=bg_)
            else:
                f_pct = f"='{A[1:-1]}'!{cl(pp_col4[src])}{rCbl}"
                if src == 'ppB':
                    gp_eff = f"-'{A[1:-1]}'!G{rEbl}"
                elif src == 'ppG':
                    gp_eff = f"'{A[1:-1]}'!C{rEbl}"
                else:
                    col_map = {'ppCOGS':5,'ppPrice':6,'ppVM':7}
                    gp_eff = f"'{A[1:-1]}'!{cl(col_map[src])}{rDbl}"
                f_idr = f"=({gp_eff})/IF({gp_start_ref}<>0,{gp_start_ref},1)"
                st(ws, row, col_pct, f_pct, fmt=F_PCT, align="right", bold=bld, bg=bg_)
                st(ws, row, col_idr, f_idr, fmt=F_PCT, align="right", bold=bld, bg=bg_)
        row += 1

    note_row(ws, row, NCOLS4,
        f"(%) = pp margin change per effect  |  (IDR%) = GP contribution of effect / GP Start per BL")
    row += 1


    # ── Conditional formatting for Tabel 1 (red/green) ───────────────────────
    from openpyxl.formatting.rule import CellIsRule
    from openpyxl.styles import PatternFill as PF2, Font as Fnt2
    red_fill  = PF2("solid", start_color="FFE0E0")
    grn_fill  = PF2("solid", start_color="E0F5E0")
    red_font  = Fnt2(name="Arial", color="CC0000", size=10)
    grn_font  = Fnt2(name="Arial", color="006400", size=10)
    t1_data_start = row - len(t4_rows)
    t1_data_end   = row - 1
    for _col in range(2, 2 + len(BLS5) * 2):
        col_letter = cl(_col)
        rng = f"{col_letter}{t1_data_start}:{col_letter}{t1_data_end}"
        ws.conditional_formatting.add(rng, CellIsRule(operator='lessThan',    formula=['0'], fill=red_fill, font=red_font))
        ws.conditional_formatting.add(rng, CellIsRule(operator='greaterThan', formula=['0'], fill=grn_fill, font=grn_font))
    row += 2

    # Set column widths for tabel 1 (baru)
    ws.column_dimensions['A'].width = 34
    for i in range(len(BLS5) * 2):
        ws.column_dimensions[cl(2 + i)].width = 12
    ...
    bridge_rows_1a = [
        ...
        ("2. Existing SKU Effect",   "existing",  "FFFFF0",True),
        ("  2.1 COGS Effect",        5,           "FFF3E0",False),
        ("  2.2 Price Effect",       6,           "E3F2FD",False),
        ("  2.3 Vol/Mix Effect",     7,           "EDE7F6",False),
    ...
    for label, src, bg_, bld in bridge_rows_1a:
        ...
        for ci, bl in enumerate(BLS+['TOTAL'], 2):
            ...
            elif src == 'existing': val_ref = f"='{A[1:-1]}'!E{rDbl}+'{A[1:-1]}'!F{rDbl}+'{A[1:-1]}'!G{rDbl}"
            elif src == 'new':      val_ref = f"='{A[1:-1]}'!C{rEbl}"
            elif src == 'total':    val_ref = f"='{A[1:-1]}'!I{rDbl}"
        ...
        for ci, bl in enumerate(BLS+['TOTAL'], 7):
            ...
            if src in ('start', 'end'):
                ...
                elif src == 'existing': num_ref = f"'{A[1:-1]}'!E{rDbl}+'{A[1:-1]}'!F{rDbl}+'{A[1:-1]}'!G{rDbl}"
                elif src == 'new':      num_ref = f"'{A[1:-1]}'!C{rEbl}"
                elif src == 'total':    num_ref = f"'{A[1:-1]}'!I{rDbl}"
    ...
    pp_rows = [
        ...
        ("2. Existing SKU Effect",       'ppExisting', "FFFFF0",True,  "sum 2.1+2.2+2.3"),
    ...
    for label, src, bg_, bld, gv_ref in pp_rows:
        ...
        for ci, bl in enumerate(BLS+['TOTAL'], 2):
            ...
            elif src == 'ppExisting': f = f"='{A[1:-1]}'!{cl(9)}{rCbl}+'{A[1:-1]}'!{cl(10)}{rCbl}+'{A[1:-1]}'!{cl(11)}{rCbl}"
    ...
    note_row(ws, row, NCOLS,
        "✓ Start + Sum pp steps = End  |  COGS & Price pp cleanest (q_P1 basis)  |  "
        "Vol/Mix = residual, absorbs interaction term")
    row += 2

    # ── Tabel 2 — Within & BL Mix Effect ─────────────────────────────────────
    sec(ws, row, NCOLS,
        "Tabel 2 — Margin Change Effect & BL Mix Effect  |  Σ Margin Change + Σ BL Mix = Overall Δmargin")
    row += 1
    hdrs_2 = ["BL","Margin P1","Margin P2","Δ Margin","GV Wt P1","GV Wt P2",
              "Δ Weight","Margin Change Effect","BL Mix Effect","",""]
    ...
    row += 1
    note_row(ws, row, NCOLS,
        "✓ Σ Margin Change (H total) + Σ BL Mix (I total) = Overall Δmargin (D total)")
    row += 2
    ...
    effects_t3 = [
        ...
        ("  2.3 Vol/Mix Effect",            'ppVM',        "EDE7F6",False),
        ("3. New SKU Effect",               'ppG',         "E8F5E9",False),
        ("Σ Margin Change (via components)","sum",          C_LIGHT, True),
        ("Margin Change Effect (direct)",   "direct",       C_LIGHT, True),
    ...
    for label, src, bg_, bld in effects_t3:
        ...
        if src is None:
            ...
            sum_row = r_t3.get('Σ Margin Change (via components)', row-2)
            ...
            for ci, bl in enumerate(BLS, 2):
                rCbl = rC[bl]; d = pvm[bl]
                if src == 'ppExisting':
                    f = (f"=('{A[1:-1]}'!{cl(9)}{rCbl}+'{A[1:-1]}'!{cl(10)}{rCbl}"
                         f"+'{A[1:-1]}'!{cl(11)}{rCbl})*{d['gv_w2']}")
                else:
                    f = f"='{A[1:-1]}'!{cl(pp_col[src])}{rCbl}*{d['gv_w2']}"
                st(ws, row, ci, f, fmt=F_PCT, align="right", bg=bg_, bold=bld)
            st(ws, row, 6, f"=SUM(B{row}:E{row})", fmt=F_PCT, align="right", bg=bg_, bold=bld)
            st(ws, row, 7, "—", align="center", fc="888888", bg=bg_)
            st(ws, row, 8, f"=F{row}", fmt=F_PCT, align="right", bg=bg_, bold=bld)
...
def write_s2b(wb, df, pvm, AGG, p1, p2):
    ...
    for label, eff, bg_, bld in bridge_defs:
        ...
        for ci, bl in enumerate(BLS + ['TOTAL'], 0):
            ...
            if label.startswith("P1"):
                ...
                first_step = step_rows[0] if step_rows else row-5
                st(ws, row, col_pos,
                   f"=SUM({cl(col_pos)}{first_step}:{cl(col_pos)}{row-1})",
                   fmt=F_NUM, align="right", bold=True, bg=bg_)
                st(ws, row, col_neg,
                   f"=SUM({cl(col_neg)}{first_step}:{cl(col_neg)}{row-1})",
                   fmt=F_NUM, align="right", bold=True, bg=bg_)
            else:
                if eff == 'existing':
                    pos_v = (sdata[bl]['cogs'][0] + sdata[bl]['price'][0] + sdata[bl]['volmix'][0])
                    neg_v = (sdata[bl]['cogs'][1] + sdata[bl]['price'][1] + sdata[bl]['volmix'][1])
                else:
                    pos_v, neg_v = sdata[bl][eff]
                st(ws, row, col_pos, pos_v, fmt=F_NUM, align="right", bg=bg_, fc="007030", bold=bld)
                st(ws, row, col_neg, neg_v, fmt=F_NUM, align="right", bg=bg_, fc="CC0000", bold=bld)
    ...
    for label, src, bg_, bld, gv_ref in pp_rows_2b:
        ...
        for ci, bl in enumerate(BLS+['TOTAL'], 2):
            ...
            elif src == 'ppExisting': f = (f"='{A[1:-1]}'!{cl(9)}{rCbl}+'{A[1:-1]}'!{cl(10)}{rCbl}"
                                           f"+'{A[1:-1]}'!{cl(11)}{rCbl}")
...
def run_pvm(file_bytes, filename):
    import io
    import pandas as pd
    import numpy as np
    ...
    wb.save(out_buf)
    out_buf.seek(0)
    return out_buf.getvalue(), f"{p1}_vs_{p2}_gp_bridge.xlsx"
