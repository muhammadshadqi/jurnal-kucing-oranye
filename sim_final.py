...
def validate_bytes(f1_bytes, f1_name, f2_bytes, f2_name, prog):
    ...
    def _clean_pid(v):
        """Force product_id to integer string: 875.0 → '875'"""
        try:
    ...
    # Force product_id to int string
    for i, r in enumerate(df1._data):
        df1._data[i]["product_id"] = _clean_pid(r.get("product_id",""))
    ...
    if ext2 == "csv":
        ...
        # Rename by position: col1=product_id, col2=baseline, col3+=var_1,var_2,...
        orig_cols2 = list(raw2[0].keys())
        n2 = len(orig_cols2)
        new_cols2 = ["product_id", "baseline"] + [f"var_{i}" for i in range(1, n2-1)]
        rows2 = []
        for r in raw2:
            orig_vals = list(r.values())
            rows2.append({new_cols2[i]: _try_num(orig_vals[i]) for i in range(min(len(new_cols2), len(orig_vals)))})
        df2r = SimpleDF(rows2, new_cols2)
    else:
        import openpyxl as _oxl
        ...
        # Rename by position regardless of original header names
        n2 = len(rows_raw2[0])
        new_cols2 = ["product_id", "baseline"] + [f"var_{i}" for i in range(1, n2-1)]
        data_rows2 = []
        for r in rows_raw2[1:]:
            if any(v is not None for v in r):
                data_rows2.append({new_cols2[i]: _try_num(r[i]) for i in range(min(len(new_cols2), len(r)))})
        df2r = SimpleDF(data_rows2, new_cols2)

    # Force product_id to int string
    for i, r in enumerate(df2r._data):
        df2r._data[i]["product_id"] = _clean_pid(r.get("product_id",""))

    # vcols = only var_* columns (exclude baseline)
    vcols = [c for c in df2r.columns if c.startswith("var_")]
    if "baseline" not in df2r.columns: raise ValueError("File 2 harus punya kolom baseline")
